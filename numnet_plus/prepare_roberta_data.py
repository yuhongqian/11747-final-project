...
parser.add_argument("--model_type", choices=["roberta", "electra"], required=True)
...
if args.model_type == "roberta":
    ...
    tokenizer = ElectraTokenizer.from_pretrained(args.input_path + "/electra.large")
...
if args.tag_mspan:
    ...
    for dm in data_mode:
        ...
        print("Save data to {}.".format(os.path.join(args.output_dir, "tmspan_cached_{}_{}.pkl".format(args.model_type, dm))))
        with open(os.path.join(args.output_dir, "tmspan_cached_{}_{}.pkl".format(args.model_type, dm)), "wb") as f:
            pickle.dump(data, f)
    ...
    for dm in data_mode:
        dpath = os.path.join(args.input_path, data_format.format(dm))
        data = train_reader._read(dpath)
        print("Save data to {}.".format(os.path.join(args.output_dir, "cached_{}_{}.pkl".format(args.model_type, dm))))
        with open(os.path.join(args.output_dir, "cached_{}_{}.pkl".format(args.model_type, dm)), "wb") as f:
            pickle.dump(data, f)
